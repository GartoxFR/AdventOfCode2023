...
fn apply_jockers(jocker_count: u32, typ: HandType) -> HandType {
    match (jocker_count, typ) {
        (0, typ) => typ,
...
fn hand_type(cards: &[Card; 5]) -> HandType {
    ...
    let joker_count = cards
        .iter()
        .take_while(|card| **card == Card::Jocker)
        .count();
    ...
    for card in cards.into_iter().skip(joker_count) {
        match last_card {
            None => {
                last_card = Some(card);
                streak = 1;
            }
            Some(last_card) if card == last_card => {
                streak += 1;
            }
            Some(_) => {
    ...
    apply_jockers(joker_count as u32, typ)
}
