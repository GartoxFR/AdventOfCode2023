...
impl Game {
    ...
    fn min_set_product(&self) -> u32 {
        self.draws
            .iter()
            .fold(Draw::default(), |acc, draw| acc.max(draw))
            .colors
            .into_values()
            .product()
    }
...
impl Draw {
    fn is_valid(&self) -> bool {
        self.colors
            .iter()
            .all(|(color, count)| *count <= color.limit())
    }
    ...
    fn max(&self, other: &Self) -> Self {
        let colors = [Color::Green, Color::Red, Color::Blue]
            ...
            .map(|color| {
                (
                    color,
                    std::cmp::max(self.colors.get(&color), other.colors.get(&color))
                        .copied()
                        .unwrap_or(0),
                )
            })
...
fn draw(input: &str) -> IResult<&str, Draw> {
    ...
    fold_many1(color_count, Draw::default, |mut draw, (count, color)| {
...
fn solve(input: &str) -> String {
    ...
    games.iter().map(Game::min_set_product).sum::<u32>().to_string()
}
